...
import { CredentialGrid_V2 } from "@/components/CredentialGrid_V2";
    ...
    <article className="pt-[72px] bg-zinc-950">
        ...
        <div className="mt-10 grid gap-10 lg:grid-cols-12 lg:items-end">
          <div className="lg:col-span-8">
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-white/40">Verified Documents Registry</span>
            </div>
            <h1 className="display mt-6 text-[clamp(3.2rem,10vw,10rem)] tracking-tight">
              <RevealLines lines={["Credentials"]} />
            </h1>
          </div>
          <Reveal delay={0.2} className="lg:col-span-4">
            <p className="text-[clamp(1.05rem,1.3vw,1.3rem)] leading-relaxed text-white/60">
              A curated archive of professional references, awards and 
              certifications. Every claim is backed by a verifiable document, 
              accessible in full to ensure complete transparency.
            </p>
          </Reveal>
        </div>
      </header>
          ...
          >
            <div className="mb-8 border-t border-line/60 pt-8 md:mb-12 flex flex-col md:flex-row md:items-end justify-between gap-4">
              <Reveal>
                ...
                {group.blurb && (
                  <p className="mt-4 max-w-[62ch] leading-relaxed text-white/50">
                    {group.blurb}
                  </p>
                )}
              </Reveal>
              <div className="hidden md:block font-mono text-[10px] uppercase tracking-widest text-white/30">
                {group.items.length} docs in category
              </div>
            </div>
            <CredentialGrid_V2 items={group.items} />
          </section>
        ))}
      </div>
      ...
      <section className="border-t border-line/60 px-5 py-16 md:px-10 md:py-24 bg-white/[0.01]">
        <Reveal>
          ...
          <p className="mt-6 max-w-[60ch] leading-relaxed text-white/60">
            Need a direct reference, the full CV, or a walkthrough of a 
            specific project? I'm available for verification and deep dives.
          </p>
